...
class Mod_video extends CI_Model {
    ...
    var $dir_img = './uploads/video/';
    
    var $dir_img_thumb = './uploads/video/thumb/';
    ...
    public function get_link(){
        return base_url().'video/'.$this->slug.'/'.$this->id;
    }
    ...
    public function search($key=''){
        $return = array();
        $arr_key = explode(' ',$key);
        $this->db->join('author', 'author.id = video.author', 'left');
        $this->db->like('LOWER(author.name)',strtolower($key));
        $this->db->or_like('LOWER(video.title)',strtolower($key));     
        if(count($arr_key)>1)
        {
            foreach($arr_key as $Val)
            {
                $this->db->or_like('LOWER(author.name)',strtolower($Val));
                $this->db->or_like('LOWER(video.title)',strtolower($Val));
              
            }
        }
        
        $this->db->select('video.*');
        $result = $this->db->get_where('video',array('available'=>1,'stock >'=>0))->result_array();
        if(!empty($result))
        {
            foreach($result as $item)
            {
    ...
    public function get_campaign(){
        $return = array();
        $result = $this->db->get_where('campaign',array('video_id'=>$this->id))->result_array();
        if(!empty($result)){
    ...
    public function get_gallery(){
        ...
        $result = $this->db->get_where('gallery',array('video_id'=>$this->id))->result_array();
        if(!empty($result)){
    ...
    public function get_count_gallery(){
        ...
        $this->db->where(array('video_id'=>$this->id));
