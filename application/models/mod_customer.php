...
class Mod_customer extends CI_Model {
    ...
    var $dir_img = './uploads/customer/';
    
    var $dir_img_thumb = './uploads/customer/thumb/';
    ...
    public function upload_image($field_name='',$max_width='',$max_height=''){
         ...
         *---------------------------------------------------*/
                    ...
                    $cfg_rsz_med['height']          = ceil($img_height/3);
    ...
    public function get_img_src($thumb=false){
        $img ='no-img.jpg';
        $path = base_url().'uploads/customer/';
        if($thumb)
        {
            $path.='thumb/';
        }
    ...
    public function get_parent_name(){
        $row = $this->db->get_where('customer',array('id'=>$this->parent))->row();
        if(!empty($row)){
            return $row->name;
        } else {
            return '';
    ...
    public function login($email,$password){
        $hash_pass = $this->hash_pass($password);
        $query = $this->db->get_where($this->table, array('email' => $email,'password'=>$hash_pass));
        $row = $query->row_array();
        if(!empty($row)){
            $this->set_value($row);
            return true;
    ...
    public function get_address_book(){
        ...
        $query = $this->db->get_where($this->mod_address_book->table,array('customer_id'=>$this->id));
        $result = $query->result_array();
        if(!empty($result)){
            foreach($result as $item){
                $ab = new Mod_address_book();
                $ab->set_value($item);
                $array_temp[] =  $ab;
            }
        }
    ...
    public function get_order(){
        ...
        $query = $this->db->get_where($this->mod_order->table,array('customer_id'=>$this->id));
        $result = $query->result_array();
        if(!empty($result)){
            foreach($result as $item){
                $ab = new Mod_order();
