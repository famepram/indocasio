...
class Mod_category extends CI_Model {
    ...
    var $dir_img = './uploads/category/';
    
    var $dir_img_thumb = './uploads/category/thumb/';
    ...
    public function get_parent_name(){
        $row = $this->db->get_where('category',array('id'=>$this->parent))->row();
        if(!empty($row)){
            return $row->name;
        } else {
            return '';
    ...
    public function get_by_parent($parent =0){
        $array_temp = array();
        $this->db->order_by('sort','asc');
    ...
    public function get_child(){
        $array_temp = array();
        $this->db->order_by('sort','asc');
        $this->db->where(array('parent'=>$this->id));
        $query = $this->db->get($this->table);
        $result = $query->result_array();
        if(!empty($result))
        {
    ...
    public function get_ids_child(){
        $ids = array();
        $children = $this->get_child();
        if(!empty($children)){
    ...
    public function get_product($from=0, $length=2,$orderby='cdate',$orderseq='desc'){

        $this->load->model('mod_product');
        $array_temp = array();
        
        if($this->parent == 0){
            $ids = $this->get_ids_child();
            $ids[] = $this->id;
            $this->db->where('publish',1);
            $this->db->where_in('category', $ids);
        } else {
            $this->db->where(array('category'=>$this->id,'publish'=>1));
        }
        $this->db->limit($length,$from);
        $this->db->order_by($orderby,$orderseq);
        
        $query = $this->db->get($this->mod_product->table);
    ...
    public function get_product_adm(){
        ...
        if($this->parent == 0){
            ...
            $this->db->where(array('category'=>$this->id));
        }
        //die($this->db->last_query()."---------------dsadasdsad-------------------dasdasdasd");
        //die("dsadasdsad-------------------dasdasdasd");
    ...
    public function get_total_prod(){
        if($this->parent == 0){
            $ids = $this->get_ids_child();
            $ids[] = $this->id;
            $this->db->where('publish',1);
    ...
    public function get_link(){
        if(!empty($this->parent)){
            ...
            return base_url().$parent->slug.'/'.$this->slug;
        } else {
