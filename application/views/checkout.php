...
class Checkout extends CI_Controller {
    ...
    public function confirmation(){
        ...
        if(isset($this->_preorder['ab_id']) && !empty($this->_preorder['ab_id'])){
            ...
            $this->costs = get_courier($this->_ab->city_id,($this->mod_cart->total_weight * 10));
        } else {

            $this->_data_load['preorder'] = $this->_preorder;
            //die(print_r($this->_data_load['preorder']));
            $this->costs = get_courier($this->_preorder['city_id'],($this->mod_cart->total_weight * 10));
        }
    ...
    public function confirmation_post(){
        ...
        if(!empty($this->_customer_id)){
            ...
            $ext_cc_cost                        = 0;
            if($payment == 2){
                $ext_cc_cost                    = round(((3.2 * $this->mod_cart->total_price /100) + 2500),0);
            }
            
            $data_order['ext_cc']               = $ext_cc_cost ;
            $data_order['delivery_cost']        = $delivery_cost;
            $data_order['total_cost']           = $data_order['total_price'] + $delivery_cost + $assurance_cost + $ext_cc_cost;
            ...
            if($this->mod_order->add()){
                ...
                $data_order = $this->mod_order->data;
                $data_order['status'] = $status;
                $this->mod_order->set_value($data_order);
                $this->mod_order->update();
                if($payment == 2){
    ...
    private function _veritrans_payment($_order_id=0){
        $sever_key = "VT-server-LMfOSursyw-pCXcpmIxDEel2";
        $end_point = "https://api.sandbox.veritrans.co.id/v2/charge";
        $total = 0;
    ...
    public function get_option_city(){
        ...
        $json_response = array('options'=>get_option_city($province));
